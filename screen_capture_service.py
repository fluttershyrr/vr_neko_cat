...
class _DummyScreenCapture:
    ...
    async def capture_active_window(self, quality=-1):
        return {"ok": False, "error": "screen_capture 不可用"}

    async def capture_game_region(self, left, top, width, height, quality=-1):
        return {"ok": False, "error": "screen_capture 不可用"}
...
if not _HAS_PIL:
    VrScreenCapture = _DummyScreenCapture
